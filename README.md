...
## Health Connect & Sideloaded APK — Restricted Settings Fix

Android 13+ **blocks Health Connect permissions** for APKs installed outside the Play Store (sideloading from GitHub Releases). This is Android's "Restricted Settings" security feature.

**One-time fix per device:**

1. Open NowBrief → go to the **Vitals** tab → tap **Connect Health Connect**
2. Android shows *"Restricted setting — unavailable"* → tap **OK** to dismiss
3. Long-press the NowBrief icon on your home screen → tap **App Info ⓘ**
4. Tap the **three-dot menu ⋮** in the top-right corner
5. Select **"Allow restricted settings"**
6. Authenticate with your PIN / fingerprint
7. Open NowBrief again → Vitals tab → **Connect Health Connect** → grant permissions ✅

> **Samsung users:** If the three-dot menu is missing, go to **Settings → Security and Privacy → Auto Blocker → turn OFF**, then repeat from step 3.

> **Easier alternative:** Install the APK using **[SAI (Split APK Installer)](https://play.google.com/store/apps/details?id=com.aefyr.sai)** from the Play Store instead of tapping the raw `.apk` file. SAI uses a session-based install which Android treats as a store install — skipping Restricted Settings entirely.
...
Kotlin · Jetpack Compose · Koin · Gemini 2.0 Flash · Open-Meteo · newsdata.io · Nominatim · DataStore · DuckDuckGo Instant API · Wikipedia REST API · [AndroidLiquidGlass](https://github.com/Kyant0/AndroidLiquidGlass) · [Shapes](https://github.com/Kyant0/Shapes)

---

The floating nav bar, toggles, and sliders throughout Settings use the real liquid-glass components — `LiquidBottomTabs`, `LiquidToggle`, `LiquidSlider` — copied verbatim from [Kyant0/AndroidLiquidGlass](https://github.com/Kyant0/AndroidLiquidGlass) (`io.github.kyant0:backdrop`) and [Kyant0/Shapes](https://github.com/Kyant0/Shapes) (`io.github.kyant0:shapes`), both Apache 2.0. Huge thanks to [Kyant0](https://github.com/Kyant0) for building and open-sourcing them — genuinely one of the best Compose effect libraries out there.
