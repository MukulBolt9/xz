...
class SettingsViewModel(
    ...
    private val dataStoreRepo: DataStoreRepository by inject()
    private val powerManager: AndroidPowerManager by inject()

    val healthSourceMode = dataStoreRepo.healthSourceMode

    fun setHealthSourceMode(mode: String) {
        viewModelScope.launch { dataStoreRepo.setHealthSourceMode(mode) }
    }

    val isStealthModeEnabled = dataStoreRepo.isStealthModeEnabled

    fun setStealthModeEnabled(enabled: Boolean) {
        viewModelScope.launch { dataStoreRepo.setStealthModeEnabled(enabled) }
    }
