...
class MainViewModel(
    ...
    /**
     * Compares semver strings, stripping pre-release suffixes before comparison.
     * e.g. "1.5-beta.1" → base "1.5"; "1.5" → base "1.5" → not newer → stable wins.
     * e.g. "1.6-beta.1" → base "1.6" > "1.5" → newer (show to beta users).
     * Stable mode (/releases/latest) never returns pre-releases so this is a safety net.
     */
    private fun isNewerVersion(latest: String, current: String): Boolean {
        // Strip pre-release suffix: "1.5-beta.1" → "1.5", "1.6.0-alpha.2" → "1.6.0"
        fun base(v: String) = v.substringBefore("-")
        val latestParts  = base(latest).split(".").mapNotNull { it.toIntOrNull() }
        val currentParts = base(current).split(".").mapNotNull { it.toIntOrNull() }
        ...
        // Same base version — a pre-release is NOT newer than a stable with same base
        val latestIsPre  = latest.contains("-")
        val currentIsPre = current.contains("-")
        return when {
            !latestIsPre && currentIsPre -> true   // stable > pre-release of same ver
            latestIsPre && !currentIsPre -> false  // pre-release NOT newer than stable
            else -> false
        }
    }
