 ...
 *
 * Inspired by the iOS 26 Liquid Glass aesthetic, adapted for Android Compose:
 * - Frosted glass pill shape with subtle inner glow ring
 * - Spring-animated sliding indicator that glides between tabs
 * - Neo mode: cyan/violet gradient accents and dark glass background
 * - Ripple-free tap feedback; scale spring on active item
 *
 * Pattern from: https://medium.com/@eduardofelipi/building-a-liquid-glass-floating-bottom-nav
 */
@Composable
fun LiquidGlassNavBar(
    ...
    val activeNeoColor = Color(0xFF22D3EE)
    val activeMat      = MaterialTheme.colorScheme.primary
    val onSurface      = MaterialTheme.colorScheme.onSurface

    // Floating glass pill — sits above content with real transparency
    Box(
        modifier = modifier
            ...
            .shadow(
                ...
                ambientColor = if (isNeo) Color(0xFF22D3EE).copy(0.30f) else Color.Black.copy(0.28f),
                spotColor   = if (isNeo) Color(0xFFA855F7).copy(0.30f) else Color.Black.copy(0.45f)
            )
    ) {
        // Glass substrate — ultra-transparent so page content bleeds through
        Box(
            modifier = Modifier
                .fillMaxSize()
                .clip(RoundedCornerShape(33.dp))
                // Very translucent: dark theme ~18% opacity, light ~22%
                .background(
                    if (isNeo)
                        Brush.linearGradient(listOf(Color(0x2E000000), Color(0x3C060B14)))
                    else
                        Brush.linearGradient(listOf(Color(0x38FFFFFF), Color(0x28F0F4FF)))
                )
                // Specular highlight ring — top edge bright, bottom edge fades
                .border(
                    width = 1.dp,
                    brush = if (isNeo)
                        Brush.linearGradient(
                            0f to Color(0x9922D3EE),
                            0.5f to Color(0x33A855F7),
                            1f to Color(0x0022D3EE)
                        )
                    else
                        Brush.linearGradient(
                            0f to Color(0xCCFFFFFF),
                            0.45f to Color(0x55FFFFFF),
            ...
            BoxWithConstraints(Modifier.fillMaxSize().padding(5.dp)) {
                val pillW = maxWidth / tabs.size
                val offsetX by animateDpAsState(
                    targetValue = pillW * selected,
                ...
                Box(
                    Modifier
                        ...
                        .background(
                            if (isNeo)
                                Brush.linearGradient(listOf(Color(0x5522D3EE), Color(0x44A855F7)))
                            else
                                Brush.linearGradient(listOf(activeMat.copy(0.22f), activeMat.copy(0.13f)))
                        )
                )
            }
            ...
            Row(
                ...
                tabs.forEachIndexed { i, label ->
                    ...
                    val itemColor by animateColorAsState(
                        targetValue = when {
                            sel && isNeo  -> activeNeoColor
                            sel           -> activeMat
                            isNeo         -> Color(0xFF94A3B8)
                            else          -> onSurface.copy(alpha = 0.50f)
                        },
                    ...
                    Column(
                        ...
                        Text(
                            tabIcons.getOrElse(i) { "\u2022" },
                            fontSize = if (sel) 20.sp else 16.sp
                        )
                        ...
                        Box(
                            Modifier
                                .size(if (sel) 4.dp else 0.dp)
                                .clip(RoundedCornerShape(2.dp))
                                .background(if (isNeo) activeNeoColor else activeMat)
                        )
                    }
                }
            }
...
fun VitalsTab() {
    ...
    val hcPermLauncher = rememberLauncherForActivityResult(
        ...
        if (grantedPermissions.isNotEmpty()) {
            coroutineScope.launch {
                ...
                // Force fresh snapshot regardless of existing hcSnap:
                // the launcher fires when user returns from HC permission screen,
                // same as the ON_RESUME path but explicit here for clarity.
                val fresh = hcRepo.snapshot()
                if (fresh != null) {
                    hcSnap = fresh
                    fresh.sleep?.primary?.let { s ->
    ...
    // ── HC permission check: fires on first open AND every time the app resumes ──
    // This is the critical fix for "I connect HC after closing the app":
    //   LaunchedEffect(Unit) alone only fires once per composition.
    //   The DisposableEffect ON_RESUME observer fires every time the app comes
    //   back from background — including returning from the HC permission screen
    //   or from HC settings after manually granting permissions.
    val lifecycleOwner = LocalLifecycleOwner.current

    // Suspend helper — called from both first-open AND every ON_RESUME.
    // Always fetches fresh — HC data changes constantly (steps, sleep, HR)
    // and the client session can expire while the app is in the background.
    suspend fun checkAndLoadHc() {
        ...
        if (!granted) return
        // Always build a fresh snapshot — never skip due to existing hcSnap.
        // This is what makes HC data appear after granting permissions while the app was closed.
        val fresh = hcRepo.snapshot()
        if (fresh != null) {
            hcSnap = fresh
            fresh.sleep?.primary?.let { s ->
    ...
    // Re-check on every app resume (catches: HC permission granted while app was in background,
    // returning from HC settings screen, device unlocked with new data, etc.)
    DisposableEffect(lifecycleOwner) {
        val observer = LifecycleEventObserver { _, event ->
            if (event == Lifecycle.Event.ON_RESUME) {
                coroutineScope.launch { checkAndLoadHc() }
            }
        }
    ...
    LazyColumn(
            ...
            Card(Modifier.fillMaxWidth(), RoundedCornerShape(24.dp),
                ...
                Column(Modifier.padding(horizontal = 16.dp, vertical = 20.dp),
                    ...
                    Row(
                        ...
                        // Night sleep ring — always visible; uses estimatedSleepH (smart detector or HC)
                        val sleepMode   = hcSnap?.sleep?.mode
                        val hasHcNapData = (hcSnap?.sleep?.napCount ?: 0) > 0 &&
                            sleepMode == com.kakao.taxi.data.repository.HealthConnectRepository.SleepDisplayMode.TODAY_NAP
                        val nightActual = hcSnap?.sleep?.primary?.actualMinutes
                        val nightHours  = if (nightActual != null && nightActual > 0) nightActual / 60f else 0f
                        // Night = HC night value if available, else smart-detector value
                        val nightSleepH = if (!hasHcNapData && nightHours > 0f) nightHours else estimatedSleepH
                        val nightSleepFrac = (nightSleepH / sleepTargetHours).coerceIn(0f, 1f)
                        val nightLabel = when {
                            isSleepingNow  -> "sleeping"
                            nightSleepH > 0f -> "/ ${if (sleepTargetHours == sleepTargetHours.toLong().toFloat()) "${sleepTargetHours.toInt()}h" else "${sleepTargetHours}h"} goal"
                            else           -> "/ ${if (sleepTargetHours == sleepTargetHours.toLong().toFloat()) "${sleepTargetHours.toInt()}h" else "${sleepTargetHours}h"} goal"
                        }
                        val nightValueText = when {
                            isSleepingNow -> {
                                val h = nightSleepH.toInt(); val m = ((nightSleepH - h) * 60).toInt()
                        ...
                        // Nap ring — ALWAYS shown (☀️), shows -- when no nap yet today
                        val napActual   = if (hasHcNapData) hcSnap!!.sleep!!.todayNaps.sumOf { it.actualMinutes } else 0L
                        val napHours    = napActual / 60f
                        Box(Modifier.width(1.dp).height(130.dp).padding(top = 8.dp)
                            .background(MaterialTheme.colorScheme.outline.copy(0.15f)))
                        val napFrac = (napHours / sleepNapTargetHours).coerceIn(0f, 1f)
                        val napGoalStr = if (sleepNapTargetHours == sleepNapTargetHours.toLong().toFloat()) "${sleepNapTargetHours.toInt()}h" else "${sleepNapTargetHours}h"
                        val napLabel = "/ $napGoalStr goal"
                        val napValueText = if (napHours > 0f) "${"%.1f".format(napHours)}h" else "--"
                        Box(Modifier.weight(1f), contentAlignment = Alignment.TopCenter) {
                            VitalRingItem(
                                progress  = napFrac,
                                value     = napValueText,
                                ...
                                emoji     = "\u2600\uFE0F",
                                ringColor = when {
                                    napFrac >= 1f   -> Color(0xFF4CAF50)
                                    napFrac >= 0.5f -> Color(0xFFFFA726)
                                    napHours > 0f   -> Color(0xFFFF7043)
                                    else            -> Color(0xFFFFA726).copy(alpha = 0.4f)
                                },
                                enabled   = true,
                                onEnable  = {}
                            )
        ...
        item {
            val hcColor = when {
                ...
                hcEverGranted         -> Color(0xFF2196F3)  // granted but loading
            ...
            Card(Modifier.fillMaxWidth(), RoundedCornerShape(22.dp),
                ...
                Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
                    ...
                    // Fallback values from smart-detector when HC has no data
                    val effSteps    = snap?.steps?.takeIf { it >= 0 }?.toInt() ?: sensorSteps.takeIf { it > 0 }
                    val effCalories = snap?.calories?.takeIf { it > 0 }
                        ?: effSteps?.let { (it * 0.04f).toInt().toDouble() }
                    val effDistKm   = snap?.distanceKm?.takeIf { it > 0 }
                        ?: effSteps?.let { it * 0.00078 }
                    val effSleepH   = snap?.sleep?.primary?.let { it.actualMinutes / 60f }
                        ?: estimatedSleepH.takeIf { it > 0f }

                    val metrics = listOf(
                        HcMetric("\uD83D\uDC5F", "Steps",
                            when {
                                snap?.steps?.let { it >= 0 } == true -> "${snap!!.steps}"
                                effSteps != null -> "$effSteps ~"
                                else -> "--"
                            },
                            effSteps != null),
                        ...
                        HcMetric("\uD83D\uDECF\uFE0F", "Sleep",
                            when {
                                snap?.sleep?.primary != null -> "${"%.1f".format(snap.sleep!!.primary!!.actualMinutes / 60f)}h"
                                effSleepH != null -> "${"%.1f".format(effSleepH)}h ~"
                                else -> "--"
                            },
                            effSleepH != null),
                        HcMetric("\uD83D\uDD25", "Calories",
                            when {
                                snap?.calories?.let { it > 0 } == true -> "${snap!!.calories.toInt()} kcal"
                                effCalories != null -> "${effCalories.toInt()} kcal ~"
                                else -> "--"
                            },
                            effCalories != null),
                        HcMetric("\uD83D\uDCCD", "Distance",
                            when {
                                snap?.distanceKm?.let { it > 0 } == true -> "${"%.2f".format(snap!!.distanceKm)} km"
                                effDistKm != null -> "${"%.2f".format(effDistKm)} km ~"
                                else -> "--"
                            },
                            effDistKm != null),
                    )
                        ...
                        Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                            ...
                            if (night != null) {
                                ...
                                if (night.hasStageData && (night.deepMinutes + night.remMinutes + night.lightMinutes) > 0) {
                                    ...
                                    Text(
                                        if (night.hasStageData) "Stage data: not recorded by wearable"
                                        else "No stage breakdown — Samsung Health sync pending",
                                        fontSize = 9.sp,
                                        color = MaterialTheme.colorScheme.onSurfaceVariant.copy(0.4f)
                                    )
                                }
                            } else {
                                // No sleep data from HC yet
                                Text(
            ...
            if (snap.calories > 0 || snap.distanceKm > 0) {
                item {
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                        if (snap.calories > 0) {
                            Card(Modifier.weight(1f), RoundedCornerShape(18.dp),
                                CardDefaults.cardColors(MaterialTheme.colorScheme.surfaceVariant),
                                CardDefaults.cardElevation(0.dp)) {
