...
// ── LiquidGlassCard — real GPU blur+vibrancy+lens via Kyant0 drawBackdrop ─
// Pattern mirrors upstream LiquidBottomTabs.kt exactly:
//   drawBackdrop(shape=..., effects={vibrancy+blur+lens}, onDrawSurface={...})
// NO .clip() before drawBackdrop — it handles its own shape-clipping internally.
@Composable
fun LiquidGlassCard(
    ...
    val isDark   = !isSystemInDarkTheme().not()
    val backdrop = LocalGlassBackdrop.current
    val shape    = RoundedCornerShape(cornerRadius)

    // Surface tint: dark = white at 10% (frosted glass haze); light = white at 45%
    val containerColor = if (isDark) Color.White.copy(alpha = 0.10f)
                         else        Color.White.copy(alpha = 0.45f)

    // Subtle specular rim (top edge only) — simulates glass catching light
    val rimColor = if (isDark) Color.White.copy(alpha = 0.18f) else Color.White.copy(alpha = 0.70f)
    ...
    if (backdrop != null) {
            ...
            modifier = modifier
                .drawBackdrop(
                    ...
                    onDrawSurface = {
                        ...
                        // Specular rim — top-edge highlight simulates glass catching light
                        drawLine(
                            rimColor,
        ...
        Box(
            modifier = modifier
                ...
                .border(
                    width = 1.dp,
                    brush = Brush.verticalGradient(listOf(rimColor, Color.Transparent)),
                    shape = shape
                )
        ) { content() }
    }
        ...
        Box(
            ...
            Canvas(Modifier.fillMaxSize()) {
                ...
                if (isNeo) {
                    ...
                    // Dark/default — vivid aurora blobs so glass refracts rich colour (boosted opacity)
                    drawCircle(Brush.radialGradient(listOf(Color(0x5500CFFF), Color(0x1800CFFF), Color.Transparent),
                        center = Offset(w * 0.20f + dx, h * 0.25f + dy), radius = w * 0.60f),
                        radius = w * 0.60f, center = Offset(w * 0.20f + dx, h * 0.25f + dy))
                    drawCircle(Brush.radialGradient(listOf(Color(0x44A78BFA), Color(0x14A78BFA), Color.Transparent),
                        center = Offset(w * 0.80f - dx, h * 0.50f - dy), radius = w * 0.55f),
                        radius = w * 0.55f, center = Offset(w * 0.80f - dx, h * 0.50f - dy))
                    drawCircle(Brush.radialGradient(listOf(Color(0x4000FFAA), Color(0x1000FFAA), Color.Transparent),
