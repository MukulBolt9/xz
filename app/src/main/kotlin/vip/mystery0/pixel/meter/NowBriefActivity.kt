...
fun AnimatedRingChart(
    ...
    Box(modifier.aspectRatio(1f), contentAlignment = Alignment.Center) {
        Canvas(Modifier.fillMaxSize()) {
            // Force a perfect square draw area using the minimum dimension
            val dim    = minOf(size.width, size.height)
            val stroke = dim * 0.095f
            val pad    = stroke / 2f + 4f
            // Center the square within canvas
    ...
    LaunchedEffect(Unit) {
        ...
        try {
            ...
            fun scoreGap(offMs: Long, onMs: Long, gapH: Float): Int {
                ...
                if (gapH >= 7f)  score += 1   // >7h = definitely a full night

                // Signal: time-of-night quality
                if (offHour in 22..23 || offHour in 0..2) score += 3   // prime bedtime 10PM–2AM
                else if (offHour in 20..21)                score += 2   // early-night
                else if (offHour in 3..6)                  score += 1   // late-night/pre-dawn
                else if (offHour in 7..11)                 score -= 1   // morning — less likely sleep
                else if (offHour in 12..19)                score -= 3   // DAYTIME: almost never night sleep

                // Signal: charging (phone parked overnight)
                if (isCharging) score += 2

                // Signal: declared habits from habits pane
                if (sleepBehaviorsNone || sleepBehaviors.isEmpty()) {
                    ...
                    for (habit in sleepBehaviors) {
                        ...
                        when {
                            h.contains("screen")   -> score += 3  // tautological — screen IS off
                            ...
                            h.contains("mobile data") || (h.contains("data") && !h.contains("dark")) -> score += 2
                ...
                // Signal: only gap this night (consistent sleeper)
            ...
            Card(Modifier.fillMaxWidth(), RoundedCornerShape(24.dp),
                ...
                Column(Modifier.padding(horizontal = 16.dp, vertical = 20.dp),
                    ...
                    Row(
                        ...
                        // Heuristic nap: only if:
                        //  • Not already HC-classified as nap
                        //  • estimatedSleepH is SHORT (< 4h) — long sleep = full night, never nap
                        //  • Current hour is DAYTIME afternoon (12–19): genuine post-lunch nap window
                        //  • We already saw a prior night session stored in DataStore (so this is second gap)
                        //    OR HC has a primary night record already (so the short gap is definitely a nap)
                        val hasHcNightData = (hcSnap?.sleep?.primary?.actualMinutes ?: 0) > 60
                        // estimatedSleepH from P2 (DataStore) is the prior-night marker if > 3h
                        // We use estimatedSleepH itself: if it's short now but we saw it >3 earlier,
                        // that prior night is already persisted. Use hcNightH as fallback.
                        val hcNightHForNap = hcSnap?.sleep?.primary?.actualMinutes?.let { if (it > 60) it / 60f else 0f } ?: 0f
                        val hasPriorNight  = hasHcNightData || hcNightHForNap > 3f
                        val isHeurNap    = !isHcNapMode &&
                            estimatedSleepH in 0.25f..3.49f &&
                            hour in 12..19 &&
                            hasPriorNight
        ...
        item {
            ...
            Card(Modifier.fillMaxWidth(), RoundedCornerShape(22.dp),
                ...
                Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
                    ...
                    val hasAnyHcData = hcSnap?.let { s ->
                        (s.steps > 0) || (s.heartRate != null) ||
                        (s.sleep != null) || (s.calories > 0)
                    } == true
                    if (hcAvailable && hcEverGranted) {
                        if (hasAnyHcData) {
                            ...
                            Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                                ...
                                Text(
                                    "Health Connect is a data bridge — it needs a paired fitness app writing into it. Choose a source below:",
                                    fontSize = 11.sp, lineHeight = 16.sp,
                                    color = MaterialTheme.colorScheme.onSurfaceVariant
                                )

                                // ── Google Account + Google Fit featured card ──────────────
                                val gFitPkg = "com.google.android.apps.fitness"
                                val gFitInstalled = try { context.packageManager.getPackageInfo(gFitPkg, 0); true } catch (_: Exception) { false }
                                Surface(
                                    shape = RoundedCornerShape(16.dp),
                                    color = Color(0xFF1A237E).copy(0.18f),
                                    modifier = Modifier.fillMaxWidth().clickable {
                                        try {
                                            if (gFitInstalled)
                                                context.startActivity(context.packageManager.getLaunchIntentForPackage(gFitPkg)?.addFlags(android.content.Intent.FLAG_ACTIVITY_NEW_TASK))
                                            else
                                                context.startActivity(android.content.Intent(android.content.Intent.ACTION_VIEW, android.net.Uri.parse("market://details?id=$gFitPkg")).addFlags(android.content.Intent.FLAG_ACTIVITY_NEW_TASK))
                                        } catch (_: Exception) {}
                                    }
                                ) {
                                    Column(Modifier.fillMaxWidth().padding(14.dp),
                                        ...
                                        Row(Modifier.fillMaxWidth(),
                                            ...
                                            Row(horizontalArrangement = Arrangement.spacedBy(10.dp),
                                                ...
                                                Column {
                                                    Row(horizontalArrangement = Arrangement.spacedBy(6.dp),
                                                        verticalAlignment = Alignment.CenterVertically) {
                                                        Text("Google Fit", fontSize = 14.sp, fontWeight = FontWeight.Bold,
                                                            color = Color(0xFF82B1FF))
                                                        Box(Modifier.clip(RoundedCornerShape(50))
                                                            .background(Color(0xFF1565C0).copy(0.3f))
                                                            .padding(horizontal = 6.dp, vertical = 2.dp)) {
                                                            Text("RECOMMENDED", fontSize = 7.5.sp,
                                                                color = Color(0xFF82B1FF), fontWeight = FontWeight.ExtraBold,
                                                                letterSpacing = 0.5.sp)
                                                        }
                                                        if (gFitInstalled) Box(Modifier.clip(RoundedCornerShape(50))
                                                            .background(Color(0xFF4CAF50).copy(0.18f))
                                                            .padding(horizontal = 6.dp, vertical = 2.dp)) {
                                                            Text("installed", fontSize = 8.sp,
                                                                color = Color(0xFF4CAF50), fontWeight = FontWeight.Bold)
                                                        }
                                                    }
                                                    Text("Steps · Calories · Heart Rate · Sleep · Distance · Active min",
                                                        fontSize = 9.5.sp, color = Color(0xFF82B1FF).copy(0.65f))
                                                }
                                            }
                                            Text(if (gFitInstalled) "Open →" else "⬇ Install",
                                                fontSize = 11.sp, fontWeight = FontWeight.SemiBold,
                                                color = Color(0xFF82B1FF))
                                        }

                                        HorizontalDivider(color = Color(0xFF1565C0).copy(0.2f))

                                        // Google Account sign-in note
                                        Surface(shape = RoundedCornerShape(10.dp),
                                            color = Color(0xFF0D47A1).copy(0.12f)) {
                                            Row(Modifier.fillMaxWidth().padding(horizontal = 10.dp, vertical = 8.dp),
                                                horizontalArrangement = Arrangement.spacedBy(8.dp),
                                                verticalAlignment = Alignment.CenterVertically) {
                                                Text("G", fontSize = 15.sp, fontWeight = FontWeight.ExtraBold,
                                                    color = Color(0xFF4285F4))
                                                Column(verticalArrangement = Arrangement.spacedBy(2.dp)) {
                                                    Text("Google Account required", fontSize = 11.sp,
                                                        fontWeight = FontWeight.SemiBold,
                                                        color = MaterialTheme.colorScheme.onSurface)
                                                    Text("Sign in to Google Fit with your Google account first, then enable Health Connect sync in Fit's settings.",
                                        ...
                                        Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                                            listOf(
                                                "1" to "Install Google Fit from Play Store",
                                                "2" to "Open Fit → sign in with your Google account",
                                                "3" to "Tap Profile → Settings → Health Connect",
                                                "4" to "Tap 'Manage permissions' → Allow all data types",
                                                "5" to "Return here → tap Refresh above"
                                            ).forEach { (num, step) ->
                                                Row(horizontalArrangement = Arrangement.spacedBy(8.dp),
                                                    verticalAlignment = Alignment.Top) {
                                                    Box(Modifier.size(18.dp).clip(CircleShape)
                                                        .background(Color(0xFF1565C0).copy(0.25f)),
                                                        contentAlignment = Alignment.Center) {
                                                        Text(num, fontSize = 9.sp, fontWeight = FontWeight.Bold,
                                                            color = Color(0xFF82B1FF))
                                                    }
                                                    Text(step, fontSize = 10.sp, lineHeight = 15.sp,
                                                        color = MaterialTheme.colorScheme.onSurfaceVariant.copy(0.7f),
                                ...
                                // ── Other bridge apps ──────────────────────────────────────
                                Text("OTHER SOURCES", fontSize = 9.sp, fontWeight = FontWeight.Bold,
                                    letterSpacing = 0.8.sp,
                                    color = MaterialTheme.colorScheme.onSurfaceVariant.copy(0.4f))
