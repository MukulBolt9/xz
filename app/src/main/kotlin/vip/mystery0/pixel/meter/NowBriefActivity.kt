...
fun VitalsTab() {
    ...
    LaunchedEffect(Unit) {
        ...
        if (hcRepo.hasPermissions()) {
            hcSnap = hcRepo.snapshot()
            // Override sleep ring with HC data if available (much more accurate than heuristic)
            hcSnap?.sleep?.night?.let { night ->
                if (night.actualMinutes > 0) {
                    estimatedSleepH = night.actualMinutes / 60f
                }
            }
        }
                ...
                item {
                    ...
                    Card(Modifier.fillMaxWidth(), RoundedCornerShape(20.dp),
                        ...
                        Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                            ...
                            // Stage breakdown (if wearable tracked stages)
                            if (night.hasStageData) {
                                HorizontalDivider(color = MaterialTheme.colorScheme.outline.copy(0.12f))
                                ...
                                Text("No stage data — wearable didn't record stages",
                                    fontSize = 10.sp,
                                    color = MaterialTheme.colorScheme.onSurfaceVariant.copy(0.5f))
                            }
                        }
