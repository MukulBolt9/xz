    ...
    Box(
        ...
        Box(
            modifier = Modifier
                ...
                .background(
                    if (isNeo)
                        Brush.linearGradient(listOf(Color(0x66000D1A), Color(0x880A0A1A)))
                    else Brush.linearGradient(listOf(surfaceColor.copy(alpha = 0.88f), surfaceColor.copy(alpha = 0.88f)))
                )
                .border(
                    width = 1.dp,
                    brush = if (isNeo)
                        Brush.linearGradient(listOf(Color(0x5522D3EE), Color(0x55A855F7)))
            ...
            BoxWithConstraints(Modifier.fillMaxSize().padding(6.dp)) {
                ...
                Box(
                    Modifier
                        ...
                        .background(
                            if (isNeo)
                                Brush.linearGradient(listOf(Color(0x3322D3EE), Color(0x33A855F7)))
                            else Brush.linearGradient(listOf(activeMat.copy(alpha = 0.15f), activeMat.copy(alpha = 0.15f)))
                        )
                )
            }
            ...
            Row(
                ...
                tabs.forEachIndexed { i, label ->
                    ...
                    val scale by animateFloatAsState(
                        targetValue = if (sel) 1.08f else 1f,
                    ...
                    Column(
                        ...
                        Text(
                            ...
                            fontSize = if (sel) 19.sp else 16.sp
                        )
                        Text(
                            label,
                            fontSize = 10.sp,
...
fun VitalsTab() {
    ...
    LaunchedEffect(Unit) {
        ...
        // Always re-check — permission could have been granted or revoked outside the app
        val granted = hcRepo.hasPermissions()
        if (granted != hcEverGranted) dataStoreRepo.setHcGranted(granted)  // sync DataStore
        if (granted) {
            hcSnap = hcRepo.snapshot()
            hcSnap?.sleep?.primary?.let { s ->
                if (s.actualMinutes > 0) estimatedSleepH = s.actualMinutes / 60f
            ...
            Card(Modifier.fillMaxWidth(), RoundedCornerShape(24.dp),
                ...
                Column(Modifier.padding(horizontal = 16.dp, vertical = 20.dp),
                    ...
                    Row(
                        ...
                        val sleepFrac = (estimatedSleepH / effectiveSleepGoal).coerceIn(0f, 1f)
                        val sleepLabel = when {
                            ...
                            sleepMode == com.kakao.taxi.data.repository.HealthConnectRepository.SleepDisplayMode.TODAY_NAP -> {
                                val g = effectiveSleepGoal
                                "nap / ${if (g == g.toLong().toFloat()) "${g.toInt()}h" else "${g}h"} goal"
                            }
                            else -> {
                                val g = effectiveSleepGoal
                                "last night / ${if (g == g.toLong().toFloat()) "${g.toInt()}h" else "${g}h"} goal"
                            }
                        }
