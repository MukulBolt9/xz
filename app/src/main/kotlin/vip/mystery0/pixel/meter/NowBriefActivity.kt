...
class NowBriefActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // ── Two-layer architecture for real QmDeve glass ─────────────
        // QmDeve's LiquidGlassView must bind to a ViewGroup that does NOT
        // contain the glass view itself (same as their demo: sibling layout).
        // We split into:
        //   Layer 0 (bgContainer) — draws ONLY the aurora/gradient background.
        //                           Glass views BIND here.
        //   Layer 1 (ComposeView) — draws all UI (cards, navbar, etc.).
        //                           Glass views LIVE here but bind to layer 0.
        val root = android.widget.FrameLayout(this)

        // Background layer — pure aurora, no cards
        val bgContainer = android.widget.FrameLayout(this).also { it.id = bgSourceId }
        val bgView = androidx.compose.ui.platform.ComposeView(this).apply {
            setContent {
                ...
                PixelPulseTheme(darkTheme = isDark, isOledTheme = isOled, isNeoTheme = isNeo) {
                    GlassBackgroundLayer(networkRepository)
                }
            }
        }
        bgContainer.addView(bgView,
            ...
            android.widget.FrameLayout.LayoutParams.MATCH_PARENT)
        root.addView(bgContainer,
            android.widget.FrameLayout.LayoutParams.MATCH_PARENT,
            android.widget.FrameLayout.LayoutParams.MATCH_PARENT)

        // Foreground layer — all UI; cards bind their glass views to bgContainer
        val fgView = androidx.compose.ui.platform.ComposeView(this).apply {
            setContent {
                val isDark by networkRepository.isDarkThemeEnabled.collectAsState()
                val isOled by networkRepository.isOledThemeEnabled.collectAsState()
                val isNeo  by networkRepository.isNeoThemeEnabled.collectAsState()
                PixelPulseTheme(darkTheme = isDark, isOledTheme = isOled, isNeoTheme = isNeo) {
                    CompositionLocalProvider(LocalGlassSource provides bgContainer) {
    ...
    companion object {
        // Stable ID for the background container so it can be looked up if needed
        val bgSourceId = android.view.View.generateViewId()
    }
}

// CompositionLocal that gives every card the correct bind target for QmDeve
val LocalGlassSource = staticCompositionLocalOf<android.widget.FrameLayout?> { null }

// Background-only Composable — aurora blobs + gradient, drawn into bgContainer.
// Cards sample this via QmDeve's AGSL shader.
@Composable
fun GlassBackgroundLayer(networkRepo: com.kakao.taxi.data.repository.NetworkRepository) {
    val isNeo by networkRepo.isNeoThemeEnabled.collectAsState()
    val inf = rememberInfiniteTransition(label = "bgAurora")
    val t by inf.animateFloat(0f, 1f,
        infiniteRepeatable(tween(20000, easing = LinearEasing)), "bgT")

    Box(Modifier.fillMaxSize()) {
        // Base gradient
        Box(Modifier.fillMaxSize().background(
            if (isNeo)
                Brush.radialGradient(listOf(Color(0xFF0F172A), Color(0xFF030712)),
                    center = Offset(300f, 0f), radius = 1200f)
            else
                Brush.verticalGradient(listOf(MaterialTheme.colorScheme.background,
        ...
        Canvas(Modifier.fillMaxSize()) {
            ...
            if (isNeo) {
                drawCircle(Brush.radialGradient(listOf(Color(0x4400CFFF), Color(0x0000CFFF), Color.Transparent),
                    Offset(w*.25f+dx, h*.30f+dy), w*.55f), w*.55f, Offset(w*.25f+dx, h*.30f+dy))
                drawCircle(Brush.radialGradient(listOf(Color(0x38E879F9), Color(0x08E879F9), Color.Transparent),
                    Offset(w*.75f-dx, h*.55f-dy), w*.50f), w*.50f, Offset(w*.75f-dx, h*.55f-dy))
                drawCircle(Brush.radialGradient(listOf(Color(0x2C00FFAA), Color(0x0800FFAA), Color.Transparent),
                    Offset(w*.50f+dx*.5f, h*.80f+dy*.5f), w*.45f), w*.45f, Offset(w*.50f+dx*.5f, h*.80f+dy*.5f))
            } else {
                drawCircle(Brush.radialGradient(listOf(Color(0x28FF9580), Color(0x08FF9580), Color.Transparent),
                    Offset(w*.20f+dx, h*.25f+dy), w*.60f), w*.60f, Offset(w*.20f+dx, h*.25f+dy))
                drawCircle(Brush.radialGradient(listOf(Color(0x22A78BFA), Color(0x06A78BFA), Color.Transparent),
                    Offset(w*.80f-dx, h*.50f-dy), w*.55f), w*.55f, Offset(w*.80f-dx, h*.50f-dy))
                drawCircle(Brush.radialGradient(listOf(Color(0x1C34D399), Color(0x0834D399), Color.Transparent),
                    Offset(w*.50f+dx*.4f, h*.78f+dy*.4f), w*.45f), w*.45f, Offset(w*.50f+dx*.4f, h*.78f+dy*.4f))
            }
        }
    }
}
...
// LocalGlassBackdrop retained for SettingsActivity (Kyant0 LiquidToggle/Slider/Button)
...
fun AuroraBackground(modifier: Modifier = Modifier, content: @Composable () -> Unit) {
    ...
    Box(modifier) {
        Canvas(Modifier.fillMaxSize()) {
            ...
            val dy = (cos(t * PI * 2).toFloat()) * h * 0.08f
            // blob 1 — cyan
            drawCircle(Brush.radialGradient(listOf(Color(0x3500CFFF), Color(0x0000CFFF), Color.Transparent),
                center = Offset(w * 0.25f + dx, h * 0.30f + dy), radius = w * 0.55f),
                radius = w * 0.55f, center = Offset(w * 0.25f + dx, h * 0.30f + dy))
            // blob 2 — purple
            drawCircle(Brush.radialGradient(listOf(Color(0x28FF00FF), Color(0x08FF00FF), Color.Transparent),
                center = Offset(w * 0.75f - dx, h * 0.55f - dy), radius = w * 0.50f),
                radius = w * 0.50f, center = Offset(w * 0.75f - dx, h * 0.55f - dy))
            // blob 3 — green-teal
            drawCircle(Brush.radialGradient(listOf(Color(0x2200FFAA), Color(0x0800FFAA), Color.Transparent),
                center = Offset(w * 0.50f + dx * 0.5f, h * 0.80f + dy * 0.5f), radius = w * 0.45f),
...
// ── LiquidGlassCard — QmDeve AGSL real refraction/dispersion shader ───────
// Uses AndroidLiquidGlassView (com.qmdeve.liquidglass:core).
// Bind happens in OnGlobalLayoutListener AFTER layout is complete so
// getWidth()/getHeight() are non-zero — fixes the previous crash.
// The LiquidGlassView captures the DecorView into a RenderNode; RenderEffects
// don't execute when drawing to a Canvas so no circular capture occurs.
@Composable
fun LiquidGlassCard(
        ...
        AndroidView(
            factory = { ctx ->
                com.qmdeve.liquidglass.widget.LiquidGlassView(ctx).apply {
                    ...
                    setBlurRadius(14f)
                    setDispersion(0.6f)
                    setRefractionHeight(18f * dp)
                    setRefractionOffset(65f * dp)

                    // Wait until fully attached AND root view is measured
                    // before calling bind() — avoids beginRecording(0,0) crash.
                            ...
                            v.viewTreeObserver.addOnGlobalLayoutListener(
                                object : ViewTreeObserver.OnGlobalLayoutListener {
                                    override fun onGlobalLayout() {
                                        val root = v.rootView as? ViewGroup ?: return
                                        if (root.width > 0 && root.height > 0) {
                                            v.viewTreeObserver
                                                .removeOnGlobalLayoutListener(this)
                                            (v as? com.qmdeve.liquidglass.widget.LiquidGlassView)
                                                ?.bind(root)
                                        }
                                    }
                                }
...
fun NowBriefScreen(repo: NowBriefRepository, networkRepo: com.kakao.taxi.data.repository.NetworkRepository) {
    ...
    Box(Modifier.fillMaxSize()) {
        ...
        // In BOTH themes we draw animated aurora blobs here so the navbar glass and all
        // other LiquidGlass elements have rich colour to refract, not flat grey/white.
        val auroraInf = rememberInfiniteTransition(label = "rootAurora")
        val auroraT by auroraInf.animateFloat(0f, 1f,
            infiniteRepeatable(tween(20000, easing = LinearEasing)), "rootAuroraT")
        Box(
            Modifier
                .fillMaxSize()
            ...
            Box(Modifier.fillMaxSize().background(
                if (isNeo) Brush.radialGradient(
                    ...
                    center = Offset(0.3f, 0f), radius = 1200f
                ) else Brush.verticalGradient(
                    listOf(MaterialTheme.colorScheme.background, MaterialTheme.colorScheme.background)
                )
            ))
            // Aurora blobs — visible in both themes, tinted appropriately
            Canvas(Modifier.fillMaxSize()) {
                val w = size.width; val h = size.height
                val dx = (sin(auroraT * PI * 2).toFloat()) * w * 0.12f
                val dy = (cos(auroraT * PI * 2).toFloat()) * h * 0.08f
                if (isNeo) {
                    drawCircle(Brush.radialGradient(listOf(Color(0x3500CFFF), Color(0x0000CFFF), Color.Transparent),
                        center = Offset(w * 0.25f + dx, h * 0.30f + dy), radius = w * 0.55f),
                        radius = w * 0.55f, center = Offset(w * 0.25f + dx, h * 0.30f + dy))
                    drawCircle(Brush.radialGradient(listOf(Color(0x28E879F9), Color(0x08E879F9), Color.Transparent),
                        center = Offset(w * 0.75f - dx, h * 0.55f - dy), radius = w * 0.50f),
                        radius = w * 0.50f, center = Offset(w * 0.75f - dx, h * 0.55f - dy))
                    drawCircle(Brush.radialGradient(listOf(Color(0x2200FFAA), Color(0x0800FFAA), Color.Transparent),
                        center = Offset(w * 0.50f + dx * 0.5f, h * 0.80f + dy * 0.5f), radius = w * 0.45f),
                        radius = w * 0.45f, center = Offset(w * 0.50f + dx * 0.5f, h * 0.80f + dy * 0.5f))
                    // Neo scan-line
                    val scanT = (auroraT * 4) % 1f
                    val y = scanT * h
                    drawLine(Brush.horizontalGradient(listOf(Color.Transparent, Color(0x4422D3EE), Color.Transparent)),
                        Offset(0f, y), Offset(w, y), 1.5f)
                } else {
                    // Light mode — warm soft blobs so glass refracts colour, not flat grey
                    drawCircle(Brush.radialGradient(listOf(Color(0x22FF9580), Color(0x08FF9580), Color.Transparent),
                        center = Offset(w * 0.20f + dx, h * 0.25f + dy), radius = w * 0.60f),
                        radius = w * 0.60f, center = Offset(w * 0.20f + dx, h * 0.25f + dy))
                    drawCircle(Brush.radialGradient(listOf(Color(0x1CA78BFA), Color(0x06A78BFA), Color.Transparent),
                        center = Offset(w * 0.80f - dx, h * 0.50f - dy), radius = w * 0.55f),
                        radius = w * 0.55f, center = Offset(w * 0.80f - dx, h * 0.50f - dy))
                    drawCircle(Brush.radialGradient(listOf(Color(0x1834D399), Color(0x0834D399), Color.Transparent),
                        center = Offset(w * 0.50f + dx * 0.4f, h * 0.78f + dy * 0.4f), radius = w * 0.45f),
                        radius = w * 0.45f, center = Offset(w * 0.50f + dx * 0.4f, h * 0.78f + dy * 0.4f))
                }
                // Ambient corner glows for Neo
                if (isNeo) {
                    drawCircle(Brush.radialGradient(listOf(Color(0x1522D3EE), Color.Transparent),
                        center = Offset(0f, 0f), radius = w * 0.55f),
                        radius = w * 0.55f, center = Offset(0f, 0f))
                    drawCircle(Brush.radialGradient(listOf(Color(0x15E879F9), Color.Transparent),
                        center = Offset(w, h), radius = w * 0.55f),
                        radius = w * 0.55f, center = Offset(w, h))
                }
            }
        }
