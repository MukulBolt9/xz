...
fun VitalsTab() {
    ...
    // Suspend helper — called from both first-open and every resume
    suspend fun checkAndLoadHc() {
        ...
        if (granted && hcSnap == null) {
            // Only fetch if we don't have data yet — avoids redundant fetches on quick bg/fg cycles
            hcSnap = hcRepo.snapshot()
            hcSnap?.sleep?.primary?.let { s ->
                if (s.actualMinutes > 0) estimatedSleepH = s.actualMinutes / 60f
            }
        } else if (granted && hcSnap != null) {
            // Already have data — refresh silently in case something changed while app was closed
            val fresh = hcRepo.snapshot()
            if (fresh != null) {
                hcSnap = fresh
                fresh.sleep?.primary?.let { s ->
            ...
            Card(Modifier.fillMaxWidth(), RoundedCornerShape(24.dp),
                ...
                Column(Modifier.padding(horizontal = 16.dp, vertical = 20.dp),
                    ...
                    Row(
                        ...
                        // Sleep — Night ring always shown; Nap ring shown separately when nap data exists
                        val sleepMode   = hcSnap?.sleep?.mode
                        val hasNapData  = (hcSnap?.sleep?.napCount ?: 0) > 0 &&
                            sleepMode == com.kakao.taxi.data.repository.HealthConnectRepository.SleepDisplayMode.TODAY_NAP
                        val nightActual = hcSnap?.sleep?.primary?.actualMinutes  // null if no HC night data
                        val nightHours  = if (nightActual != null && nightActual > 0) nightActual / 60f else 0f
                        val napActual   = if (hasNapData) hcSnap!!.sleep!!.todayNaps.sumOf { it.actualMinutes } else 0L
                        val napHours    = napActual / 60f

                        // Night sleep ring
                        val nightSleepH  = if (hasNapData) nightHours else estimatedSleepH
                        val nightSleepFrac = (nightSleepH / sleepTargetHours).coerceIn(0f, 1f)
                        val nightLabel = when {
                            isSleepingNow -> "sleeping \uD83D\uDCA4"
                            nightSleepH <= 0f && hour < 20 -> "no data"
                            nightSleepH <= 0f -> "tap to set"
                        ...
                        // Nap ring — only shown when nap session detected today
                        if (hasNapData) {
                            Box(Modifier.width(1.dp).height(130.dp).padding(top = 8.dp)
                                .background(MaterialTheme.colorScheme.outline.copy(0.15f)))
                            val napFrac = (napHours / sleepNapTargetHours).coerceIn(0f, 1f)
                            val napLabel = "${if (sleepNapTargetHours == sleepNapTargetHours.toLong().toFloat()) "${sleepNapTargetHours.toInt()}h" else "${sleepNapTargetHours}h"} goal"
