    ...
    Box(Modifier.fillMaxSize().background(
        ...
        Column(Modifier.fillMaxSize()) {
            ...
            Box(Modifier.fillMaxSize()) {
                HorizontalPager(
                    state = pager,
                    Modifier.fillMaxSize().padding(bottom = 106.dp)
                ) { page ->
                    when (page) {
        ...
        Box(
            modifier = Modifier
                ...
                .border(
                    width = 1.dp,
                    brush = if (isNeo)
                        Brush.linearGradient(0f to Color(0x9922D3EE), 0.5f to Color(0x33A855F7), 1f to Color(0x0022D3EE))
                    else
                        Brush.linearGradient(0f to Color(0xCCFFFFFF), 0.45f to Color(0x55FFFFFF), 1f to Color(0x18FFFFFF)),
                    shape = RoundedCornerShape(45.dp)
                )
        ) {
            BoxWithConstraints(Modifier.fillMaxSize().padding(5.dp)) {
                ...
                Box(
                    Modifier.offset(x = offsetX).width(pillW).fillMaxHeight()
            ...
            Row(
                ...
                tabs.forEachIndexed { i, label ->
                    ...
                    val itemColor by animateColorAsState(
                        ...
                        animationSpec = tween(200), label = "lgItemColor$i"
                    )
                    ...
                    Column(
                        ...
                        Text(tabIcons.getOrElse(i) { "\u2022" }, fontSize = if (sel) 20.sp else 16.sp)
                        Spacer(Modifier.height(1.dp))
                        Text(label, fontSize = 10.sp,
                            fontWeight = if (sel) FontWeight.Bold else FontWeight.Normal,
                            color = itemColor, maxLines = 1)
                        Spacer(Modifier.height(2.dp))
                        Box(Modifier.size(if (sel) 4.dp else 0.dp).clip(RoundedCornerShape(2.dp))
...
private fun SleepDoubleRing(
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(4.dp)) {
        Box(Modifier.size(108.dp).clickable(onClick = onEditGoal), contentAlignment = Alignment.Center) {
            Canvas(Modifier.fillMaxSize()) {
                val outerS = size.minDimension * 0.092f
                val innerS = size.minDimension * 0.072f
                val oP = outerS / 2f + 4f
                val iP = oP + outerS + 3f
                val oTL = androidx.compose.ui.geometry.Offset(oP, oP)
                val oSz = androidx.compose.ui.geometry.Size(size.width - oP * 2, size.height - oP * 2)
                val oR  = size.minDimension / 2f - oP
                val iTL = androidx.compose.ui.geometry.Offset(iP, iP)
                val iSz = androidx.compose.ui.geometry.Size(size.width - iP * 2, size.height - iP * 2)
                val iR  = size.minDimension / 2f - iP
                // outer track + fill
                drawArc(nightColor.copy(0.13f), -90f, 360f, false, oTL, oSz, style = Stroke(outerS, cap = StrokeCap.Round))
                if (animNight > 0.01f) drawArc(nightColor, -90f, animNight * 360f, false, oTL, oSz, style = Stroke(outerS, cap = StrokeCap.Round))
                if (animNight > 0.01f) {
                    val a = (-Math.PI / 2.0 + animNight * Math.PI * 2.0)
                    val tx = (center.x + oR * kotlin.math.cos(a)).toFloat()
                    val ty = (center.y + oR * kotlin.math.sin(a)).toFloat()
                    drawCircle(nightColor.copy(alpha = 0.30f * pulse), outerS * 0.9f, androidx.compose.ui.geometry.Offset(tx, ty))
                    drawCircle(Color.White, outerS * 0.38f, androidx.compose.ui.geometry.Offset(tx, ty))
                    drawCircle(nightColor.copy(0.9f), outerS * 0.26f, androidx.compose.ui.geometry.Offset(tx, ty))
                }
                // inner track + fill
                drawArc(napColor.copy(0.12f), -90f, 360f, false, iTL, iSz, style = Stroke(innerS, cap = StrokeCap.Round))
                if (animNap > 0.01f) drawArc(napColor, -90f, animNap * 360f, false, iTL, iSz, style = Stroke(innerS, cap = StrokeCap.Round))
                if (animNap > 0.01f && hasNap) {
                    val a = (-Math.PI / 2.0 + animNap * Math.PI * 2.0)
                    val tx = (center.x + iR * kotlin.math.cos(a)).toFloat()
                    val ty = (center.y + iR * kotlin.math.sin(a)).toFloat()
                    drawCircle(napColor.copy(alpha = 0.25f * pulse), innerS * 0.9f, androidx.compose.ui.geometry.Offset(tx, ty))
                    drawCircle(Color.White, innerS * 0.38f, androidx.compose.ui.geometry.Offset(tx, ty))
                    drawCircle(napColor.copy(0.9f), innerS * 0.26f, androidx.compose.ui.geometry.Offset(tx, ty))
                }
            }
            Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(1.dp)) {
                Text(nightValue, fontSize = 16.sp, fontWeight = FontWeight.ExtraBold,
            ...
            Card(Modifier.fillMaxWidth(), RoundedCornerShape(24.dp),
                ...
                Column(Modifier.padding(horizontal = 16.dp, vertical = 20.dp),
                    ...
                    Row(
                        ...
                        // Concentric Sleep+Nap double ring
                        // Outer=Night (purple), Inner=Nap (amber). No Samsung Health.
                        // Nap: HC TODAY_NAP mode OR heuristic (<4h after 9am)
                        ...
                        val nightSleepH  = if (isHcNapMode) 0f else if (hcNightH > 0f) hcNightH else estimatedSleepH
                        val nightColor   = when {
                        ...
                        Box(Modifier.weight(2f), contentAlignment = Alignment.TopCenter) {
                            SleepDoubleRing(
                    ...
                    if (hcAvailable && hcEverGranted) {
                        if (hasAnyHcData) {
                            ...
                            Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                                ...
                                Text(
                                    "Health Connect is a bridge \u2014 it needs a fitness app to write data into it. Open one of these and enable Health Connect sync in its settings:",
                                    fontSize = 11.sp, lineHeight = 16.sp,
                                    color = MaterialTheme.colorScheme.onSurfaceVariant
                                )
                                data class HcSrc(val emoji: String, val name: String, val hint: String, val pkg: String, val uri: String)
                                listOf(
                                    HcSrc("\uD83D\uDCCA","Google Fit","Profile \u2192 Settings \u2192 Health Connect","com.google.android.apps.fitness","https://fit.google.com"),
                                    HcSrc("\uD83D\uDC99","Fitbit","Today \u2192 \u2699\uFE0F \u2192 Health Connect","com.fitbit.FitbitMobile","fitbit://settings"),
                                    HcSrc("\u26A1","Samsung Health","Menu \u2192 Settings \u2192 Connected services \u2192 HC","com.sec.android.app.shealth",""),
                                    HcSrc("\uD83C\uDFC3","Strava","Settings \u2192 Health Connect \u2192 Turn on","com.strava",""),
                                ).forEach { src ->
                                    val installed = try { context.packageManager.getPackageInfo(src.pkg, 0); true } catch (_: Exception) { false }
                                    Surface(shape = RoundedCornerShape(12.dp),
                                        color = if (installed) MaterialTheme.colorScheme.surface else MaterialTheme.colorScheme.surface.copy(alpha = 0.5f),
                                        ...
                                        Row(Modifier.fillMaxWidth().padding(horizontal = 12.dp, vertical = 9.dp),
                                            ...
                                            Row(horizontalArrangement = Arrangement.spacedBy(10.dp), verticalAlignment = Alignment.CenterVertically) {
                                                ...
                                                Column {
                                                    Row(horizontalArrangement = Arrangement.spacedBy(6.dp), verticalAlignment = Alignment.CenterVertically) {
                                                        Text(src.name, fontSize = 12.sp, fontWeight = FontWeight.SemiBold,
                                                            color = if (installed) MaterialTheme.colorScheme.onSurface else MaterialTheme.colorScheme.onSurfaceVariant.copy(0.5f))
                                                        if (installed) Text("installed", fontSize = 9.sp, color = Color(0xFF4CAF50), fontWeight = FontWeight.Medium)
                                                    }
                                                    Text(src.hint, fontSize = 10.sp, color = MaterialTheme.colorScheme.onSurfaceVariant.copy(0.55f), lineHeight = 14.sp)
                                                }
                                            }
                                            Text(if (installed) "\u2192" else "\u2B07\uFE0F", fontSize = 14.sp, color = MaterialTheme.colorScheme.onSurfaceVariant.copy(0.4f))
                                        }
                                    }
                                }
                                Text(
                                    "\u2139\uFE0F Once done, tap Refresh above to pull your data.",
