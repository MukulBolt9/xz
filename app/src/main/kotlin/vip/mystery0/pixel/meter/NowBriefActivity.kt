...
// LocalGlassBackdrop: used by SettingsActivity for Kyant0 LiquidToggle/Slider/Button.
// Cards use QmDeve's AGSL shader (LiquidGlassCard) and no longer read this.
...
// ── Main LiquidGlassCard — powered by QmDeve/AndroidLiquidGlassView ────
// Uses the real AGSL refraction+dispersion shader (com.qmdeve.liquidglass:core).
// Requires Android 13 (API 33); degrades gracefully to frosted fallback below.
@Composable
fun LiquidGlassCard(
    ...
    Box(
        modifier = modifier
            .shadow(
                ...
                spotColor   = Color.Black.copy(if (isDark) 0.50f else 0.25f)
            )
            .clip(shape)
    ) {
        // ── Real AGSL liquid-glass layer (QmDeve shader) ─────────────
        AndroidView(
            factory = { ctx ->
                com.qmdeve.liquidglass.widget.LiquidGlassView(ctx).apply {
                    val dp = ctx.resources.displayMetrics.density
                    setCornerRadius(cornerRadius.value * dp)
                    setBlurRadius(blurRadius)
                    setDispersion(dispersion)
                    setRefractionHeight(refractionHeight * dp)
                    setRefractionOffset(refractionOffset * dp)
                    // Bind to window root so the AGSL shader samples everything
                    // behind this card (background + aurora blobs + text below).
                    post { bind(rootView.rootView as ViewGroup) }
                }
            },
            update = { view ->
                val dp = view.resources.displayMetrics.density
                view.setCornerRadius(cornerRadius.value * dp)
        ...
        // ── Specular highlights drawn on top of the shader glass ─────
        if (shimmer) {
            Spacer(
                Modifier
                    .matchParentSize()
                    .drawWithCache {
                        ...
                        onDrawWithContent {
                            drawContent()
                            // Top-left crescent specular
                            drawOval(
                                brush = Brush.radialGradient(
                                    colorStops = arrayOf(
                                        0.00f to Color(0x60FFFFFF),
                                        0.38f to Color(0x18FFFFFF),
                                        1.00f to Color(0x00FFFFFF)
                                    ),
                                    center = Offset(w * 0.30f, -h * 0.06f),
                                    radius = w * 0.80f
                                ),
                                topLeft = Offset(-w * 0.15f, -h * 0.30f),
                                size    = Size(w * 1.30f, h * 0.60f)
                            )
                            // Pure white top rim
                            drawRect(
                                ...
                                topLeft = Offset(0f, 0f), size = Size(w, 1.4f)
                            )
                            drawRect(
                                brush = Brush.linearGradient(
                                    listOf(Color(0x00FFFFFF), Color(0x28FFFFFF), Color(0x00FFFFFF)),
                                    start = Offset(0f, 0f), end = Offset(w, 0f)
                                ),
                                topLeft = Offset(0f, 2f), size = Size(w, 0.9f)
                            )
                            // Subtle left edge catch-light
                            drawRect(
                                brush = Brush.linearGradient(
                                    listOf(Color(0x70FFFFFF), Color(0x18FFFFFF), Color(0x00FFFFFF)),
                                    start = Offset(0f, 0f), end = Offset(0f, h)
                                ),
                                topLeft = Offset(0f, 0f), size = Size(1.0f, h)
                            )
                        }
                    }
            )
