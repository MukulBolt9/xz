...
// Provides rootBackdrop to cards so they can sample the aurora layer with real blur+lens.
...
// ── Main LiquidGlassCard — real blur+refraction via Kyant0 drawBackdrop ──
@Composable
fun LiquidGlassCard(
    ...
    // Frosted fill — shows through blur when backdrop is active
    val fillAlpha = if (isDark) 0.08f else 0.28f
    ...
    Box(
        modifier = modifier
            .shadow(
                elevation = 20.dp,
                shape = shape,
                ambientColor = accentColor.copy(alpha = if (isDark) 0.25f else 0.12f),
                spotColor   = Color.Black.copy(if (isDark) 0.45f else 0.22f)
            )
            .clip(shape)
            .then(
                if (backdrop != null) {
                    // Real AGSL: vibrancy + Gaussian blur + edge lens refraction
                    Modifier.drawBackdrop(
                        ...
                        effects = {
                            ...
                            blur(18.dp.toPx())
                            lens(
                                refractionHeight = (cornerRadius * 0.6f).toPx(),
                                refractionAmount = 8.dp.toPx(),
                                chromaticAberration = true
                            )
                        },
                        onDrawSurface = {
                            drawRect(Color.White.copy(alpha = fillAlpha))
                        }
                    )
                } else {
                    // Pre-API31 / no backdrop scope fallback
                    Modifier.background(Color.White.copy(alpha = if (isDark) 0.10f else 0.40f))
                }
            )
            .drawWithContent {
                drawContent()
                ...
                drawOval(
                    brush = Brush.radialGradient(
                        colorStops = arrayOf(
                            ...
                            0.40f to Color(0x14FFFFFF),
                            1.00f to Color(0x00FFFFFF)
                        ),
                        center = Offset(w * 0.30f, -h * 0.05f),
                        radius = w * 0.78f
                    ),
                    topLeft = Offset(-w * 0.12f, -h * 0.28f),
                    size    = Size(w * 1.24f, h * 0.56f)
                )
                // Bright top rim
                drawRect(
                    brush = Brush.linearGradient(
                        listOf(Color(0x00FFFFFF), Color(0xCCFFFFFF), Color(0x00FFFFFF)),
                        start = Offset(0f, 0f), end = Offset(w, 0f)
                    ),
                    topLeft = Offset(0f, 0f), size = Size(w, 1.2f)
                )
                // Left edge catch-light
                drawRect(
                    brush = Brush.linearGradient(
                        listOf(Color(0x55FFFFFF), Color(0x10FFFFFF), Color(0x00FFFFFF)),
                        start = Offset(0f, 0f), end = Offset(0f, h)
                    ),
                    topLeft = Offset(0f, 0f), size = Size(0.9f, h)
                )
            }
    ) { content() }
}
