...
private fun SleepDoubleRing(
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(4.dp)) {
        Box(
            Modifier.size(116.dp).aspectRatio(1f).clickable(onClick = onEditGoal),
            contentAlignment = Alignment.Center
        ) {
            Canvas(Modifier.fillMaxSize()) {
...
fun NewsTab(news: List<NewsArticle>, isLoading: Boolean, isNeo: Boolean = false, onRefresh: () -> Unit = {}, onArticleClick: (String) -> Unit) {
    ...
    Box(
        ...
        Box(Modifier.align(Alignment.BottomCenter).padding(horizontal = 16.dp, vertical = 6.dp).fillMaxWidth()) {
