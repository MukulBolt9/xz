...
fun LiquidGlassCard(
    ...
    val isDark  = !isSystemInDarkTheme().not()   // true when dark
    val backdrop = LocalGlassBackdrop.current
    val shape   = RoundedCornerShape(cornerRadius)

    val containerColor = if (isDark) Color(0xFF121212).copy(0.25f)
                         else        Color(0xFFFAFAFA).copy(0.35f)

    if (backdrop != null) {
        // ── Real path: AGSL vibrancy + blur + lens refraction ────────
        Box(
            modifier = modifier
                .drawBackdrop(
                    backdrop = backdrop,
        ...
        // ── Fallback: pre-API31 or no backdrop in scope ───────────────
        Box(
            modifier = modifier
                .clip(shape)
