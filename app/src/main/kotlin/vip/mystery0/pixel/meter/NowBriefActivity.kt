...
fun VitalsTab() {
    ...
    // ── DataStore & coroutines (shared by sleep + hydration) ──────────
    val dataStoreRepo = remember { DataStoreRepository(context.dataStore) }
    val coroutineScope = rememberCoroutineScope()

    // Fetch HC snapshot once on open
    LaunchedEffect(Unit) {
        hcAvailable = hcRepo.isAvailable
        if (hcAvailable && hcRepo.hasPermissions()) {
            ...
            item {
                Card(Modifier.fillMaxWidth(), RoundedCornerShape(20.dp),
                    ...
                    Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                        ...
                        if (hcAvailable && hcSnap == null) PermissionRow("\u2764\uFE0F", "Health Connect", "Steps, sleep stages, heart rate & more") {
                            // Open Health Connect app's own permission settings screen
