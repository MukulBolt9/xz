...
fun LiquidGlassCard(
    ...
    val overlayArgb = if (isDark)
        android.graphics.Color.argb(26, 255, 255, 255)
    else
        ...
        AndroidView(
            factory = { ctx ->
                com.qmdeve.liquidglass.widget.LiquidGlassView(ctx).apply {
                    ...
                    runCatching { javaClass.getMethod("setCornerRadius", Float::class.java).invoke(this, cornerPx) }
                    runCatching { javaClass.getMethod("setCornerRadius", Int::class.java).invoke(this, cornerPx.toInt()) }
                    runCatching { javaClass.getDeclaredField("mCornerRadius").also { it.isAccessible = true }.set(this, cornerPx) }
                    runCatching { javaClass.getMethod("setOverlayColor", Int::class.java).invoke(this, overlayArgb) }
                    runCatching { javaClass.getDeclaredField("overlayColor").also { it.isAccessible = true }.set(this, overlayArgb) }
                    isClickable = false
                    isFocusable = false
                    elevation = 0f
                    background = null
            ...
            update = { view ->
                runCatching { view.javaClass.getMethod("setCornerRadius", Float::class.java).invoke(view, cornerPx) }
                runCatching { view.javaClass.getMethod("setOverlayColor", Int::class.java).invoke(view, overlayArgb) }
            },
