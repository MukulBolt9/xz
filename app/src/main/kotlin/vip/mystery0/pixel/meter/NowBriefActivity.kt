...
fun LiquidGlassCard(
    ...
    // Body — clean frosted glass (pure white/black, no colour tint)
    val bodyAlpha = if (isDark) 0x1A else 0x26
    val bodyFill = Brush.linearGradient(
        colorStops = arrayOf(
            0.00f to Color(bodyAlpha shl 24 or 0xFFFFFF),
            0.50f to Color((bodyAlpha / 2) shl 24 or 0xFFFFFF),
            1.00f to Color((bodyAlpha / 3) shl 24 or 0xFFFFFF)
        ),
        start = Offset(0f, 0f),
            ...
            .drawWithContent {
                ...
                drawOval(
                    brush = Brush.radialGradient(
                        colorStops = arrayOf(
                            0.00f to Color(0x70FFFFFF),
                            0.38f to Color(0x22FFFFFF),
                            0.65f to Color(0x08FFFFFF),
                ...
                // ── Top rim — pure white highlight ────────────────────
                drawRect(
                    brush = Brush.linearGradient(
                        listOf(Color(0x00FFFFFF), Color(0xC8FFFFFF), Color(0x00FFFFFF)),
                        start = Offset(0f, 0f), end = Offset(w, 0f)
                    ),
                    topLeft = Offset(0f, 0f), size = Size(w, 1.5f)
                )
                drawRect(
                    brush = Brush.linearGradient(
                        listOf(Color(0x00FFFFFF), Color(0x30FFFFFF), Color(0x00FFFFFF)),
                        start = Offset(0f, 0f), end = Offset(w, 0f)
                    ),
                    topLeft = Offset(0f, 2f), size = Size(w, 1f)
                )

                // ── Left & right edge lines (pure white, no colour) ───
                drawRect(
                    brush = Brush.linearGradient(
                        listOf(Color(0x88FFFFFF), Color(0x20FFFFFF), Color(0x00FFFFFF)),
                        start = Offset(0f, 0f), end = Offset(0f, h)
                    ),
                    topLeft = Offset(0f, 0f), size = Size(1.2f, h)
                )
                drawRect(
                    brush = Brush.linearGradient(
                        listOf(Color(0x00FFFFFF), Color(0x18FFFFFF), Color(0x00FFFFFF)),
                        start = Offset(0f, 0f), end = Offset(0f, h)
                    ),
                    topLeft = Offset(w - 1.2f, 0f), size = Size(1.2f, h)
                )
            }
    ) { content() }
}
